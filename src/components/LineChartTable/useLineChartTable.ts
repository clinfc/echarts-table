...
import { onUnmounted, reactive, readonly, ref, shallowRef, toRaw, watch, watchEffect, type Ref } from 'vue'
...
export function useLineChartTable(container: Ref<HTMLDivElement>, option: UseLineChartTableOpts = {}) {
  ...
  watchEffect(() => {
    const { size: ets, edge, colLabel, rowLabel } = config
    const { width, height, colWidth, rowHeight } = ets
    const { top = 2, left = 0, right = 0, bottom = 2 } = edge

    const cols = colLabel.length
    const rows = rowLabel.length
  ...
  watchEffect(() => {
    const {
      rowLabel,
      colLabel,
      edge: { top = 0, left = 0, right = 0 },
    } = config

    const grid: Opts['grid'] = []
    const xAxis: Opts['xAxis'] = []

    rowLabel.forEach((_, index) => {
      grid.push({
        left: left,
        right: right,
  ...
  watchEffect(() => {
    const { data, rowLabel } = config

    const series: Opts['series'] = []
    const yAxis: Opts['yAxis'] = []

    rowLabel.forEach((label, index) => {
